...
3. **Authorization**: `AccessPolicy.Spec.Rules[].Tools` translates to Common Expression Language (CEL) predicates (e.g., `request.headers['x-mcp-toolname'] in ['add']`) which Authorino natively evaluates.
